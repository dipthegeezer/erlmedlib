...
-module(erlmedlib_filehandler_resource).
-export([init/1]).
...
accept_content(ReqData, Context) ->
    ...
    case write_streamed_body(FP, wrq:stream_req_body(ReqData, 3), []) of
        ok -> {true, ReqData1, Context};
        Err -> {{error, Err}, ReqData1, Context}
    end.

write_streamed_body(FP, {Hunk,done}, Acc) ->
    io:format("RECEIVED ~p~n",[Hunk]),
    Value = iolist_to_binary(lists:reverse([Hunk|Acc])),
    file:write_file(FP, Value);
write_streamed_body(FP, {Hunk, Next},Acc) ->
    io:format("RECEIVED ~p~n",[Hunk]),
    write_streamed_body(FP, Next(), [Hunk|Acc]).
...
provide_content(ReqData, Context) ->
    case maybe_fetch_object(Context, wrq:disp_path(ReqData)) of
        {true, NewContext} ->
            ...
            {true, wrq:set_resp_body({stream, send_streamed_body(Body,4)},ReqData), Context};
        {false, NewContext} ->
